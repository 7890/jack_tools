...
'jack_playfile' can read and play local audio files only. There is currently no support for playlists or handling of song meta data.
...
This document refers to 'jack_playfile' version 0.86.
...
This is done by trying to open (up to a certain degree) every file. If it looks like being an audio file, the filename (and path, equal to provided input) is printed to the screen (stdout).
...
Last Update: Sun Nov 22 16:09:51 CET 2015
